...
export const useEstoque = (currentUser, setCurrentUser) => {
    ...
    const handleUpdateProduct = async (e, adminName) => {
        ...
        const fieldsToCompare = {
            nome: 'Nome', marca: 'Marca', categoria: 'Categoria', destaque: 'Destaque na Home', is_offer: 'Oferta',
        ...
        for (const key in fieldsToCompare) {
            ...
            if (String(oldValue ?? '') !== String(newValue ?? '')) {
                const displayOld = ['destaque', 'is_offer'].includes(key) ? (!!oldValue ? 'Sim' : 'Não') : (oldValue ?? 'N/A');
                const displayNew = ['destaque', 'is_offer'].includes(key) ? (!!newValue ? 'Sim' : 'Não') : (newValue ?? 'N/A');
